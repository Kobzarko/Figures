...
namespace Figures
{
    class Romb : GeoFigure
    {
        ...
        public Romb(string figure, double height) : base(figure)  // для периметра
        {
            
            _height = height; // прописуем в конструктор тут а объявляем в Figure и чудо работает!
        }

        public Romb(string figure, double diagonal1, double diagonal2): base(figure)  // для площади
        {
            _diagonal1 = diagonal1;
            _diagonal2 = diagonal2;
        }

        public Romb(string figure,double diagonal1, double diagonal2,double height) : base(figure) // для всего
        {
            _diagonal1 = diagonal1;
            _diagonal2 = diagonal2;
            _height = height;
